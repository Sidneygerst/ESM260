...
Rsqrd <- adult_young_merge %>% 
  group_by(organism ) %>% 
  summarize(R2 = summary(lm(recruit_density ~ density))$r.squared)

Rsqrd <- mutate(Rsqrd, x = 16, y = 100, Label = paste("R^2=", round(R2, 4)))
#[1] 0.48485195 0.47465074 0.06460801 0.62243309
...
The Spotted Damselfish is the most correlated between young and adults of the four fish types, and the Surgeonfish is the least. 
...
adultr <- adult_young_merge %>% 
  group_by(organism ) %>% 
  summarize(R2 = summary(lm(density ~ microhabitat_100m))$r.squared)
...
The Yellow Dameslfish is the most correlated. 
